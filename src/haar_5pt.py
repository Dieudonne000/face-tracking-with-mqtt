...
from .mediapipe_compat import get_face_mesh
...
class Haar5ptDetector:
    def __init__(
        ...
        # Downscale factor for Haar detection (0.5 = half resolution → ~2x faster)
        self.detect_scale = float(max(0.2, min(1.0, detect_scale)))
        ...
        try:
            ...
            raise RuntimeError(
                f"MediaPipe face mesh failed: {e}\n"
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 1) -> List[FaceKpsBox]:
        ...
        # Downscale for faster Haar detection
        if self.detect_scale < 1.0:
            small_w = int(W * self.detect_scale)
            small_h = int(H * self.detect_scale)
            gray_small = cv2.cvtColor(
                cv2.resize(frame_bgr, (small_w, small_h), interpolation=cv2.INTER_LINEAR),
                cv2.COLOR_BGR2GRAY,
            )
            # Adjust min_size for downscaled image
            scaled_min = (
                max(20, int(self.min_size[0] * self.detect_scale)),
                max(20, int(self.min_size[1] * self.detect_scale)),
            )
            faces = self.face_cascade.detectMultiScale(
                gray_small,
                scaleFactor=1.1,
